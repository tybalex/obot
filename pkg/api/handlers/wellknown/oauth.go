...
// OAuthAuthorizationServer represents the response from /.well-known/oauth-authorization-server
...
func (h *handler) oauthAuthorization(req api.Context) error {
	// Create a default OAuth authorization server metadata
	// This should be customized based on the actual implementation
	server := OAuthAuthorizationServer{
		Issuer:                        h.baseURL,
		AuthorizationEndpoint:         fmt.Sprintf("%s/oauth/authorize", h.baseURL),
		TokenEndpoint:                 fmt.Sprintf("%s/oauth/token", h.baseURL),
		RegistrationEndpoint:          fmt.Sprintf("%s/oauth/register", h.baseURL),
		IntrospectionEndpoint:         fmt.Sprintf("%s/oauth/introspect", h.baseURL),
		RevocationEndpoint:            fmt.Sprintf("%s/oauth/revoke", h.baseURL),
		JWKSURI:                       fmt.Sprintf("%s/.well-known/jwks.json", h.baseURL),
		ResponseTypesSupported:        []string{"code"},
		GrantTypesSupported:           []string{"authorization_code", "refresh_token"},
		CodeChallengeMethodsSupported: []string{"S256"},
	}

	// Return the server metadata as JSON
