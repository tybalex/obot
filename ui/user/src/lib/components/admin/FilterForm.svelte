<script lang="ts">
	...
	let mcpServersTableData = $derived.by(() => {
		if (mcpServersMap && mcpEntriesMap) {
			return convertMcpServersToTableData(filter.servers ?? []);
		}
		return [];
	});

	let filterUrlTableData = $derived(
		filter.urls.map((url) => ({
			id: url,
			url
		}))
	...
	function convertMcpServersToTableData(servers: string[]) {
		return servers.map((serverId) => {
			const entryMatch = mcpEntriesMap.get(serverId);
			const serverMatch = mcpServersMap.get(serverId);
...
>
	<div class="flex grow flex-col gap-4" out:fly={{ x: -100, duration }} in:fly={{ x: -100 }}>
		...
		<div class="flex flex-col gap-2">
			...
			<Table data={filterUrlTableData} fields={['url']} noDataMessage="No URLs added.">
				{#snippet actions(d)}
					<button
						class="icon-button hover:text-red-500"
						onclick={() => {
							filter.urls = filter.urls?.filter((url) => url !== d.id) ?? [];
						}}
			...
			<Table data={mcpServersTableData} fields={['name']} noDataMessage="No MCP servers added.">
				{#snippet actions(d)}
					<button
						class="icon-button hover:text-red-500"
						onclick={() => {
							filter.servers = filter.servers?.filter((server) => server !== d.id) ?? [];
						}}
	...
	<div class="flex w-full justify-end gap-2">
		...
		<button
			...
			onclick={async () => {
				...
				filter.urls = [...(filter.urls ?? []), addingUrl];
				addingUrl = '';
				closeAddUrlDialog();
			}}
		>
...
<SearchMcpServers
	...
	exclude={filter.servers ?? []}
	onAdd={async (mcpCatalogEntryIds, mcpServerIds) => {
		filter.servers = [...(filter.servers ?? []), ...mcpCatalogEntryIds, ...mcpServerIds];
	}}
/>
