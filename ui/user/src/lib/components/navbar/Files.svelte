<script lang="ts">
	import { FileText, Trash, Wrench } from '$lib/icons';
...
<button use:ref class="z-20 icon-button" onclick={async () => {
	await loadFiles();
	toggle()
}}
				type="button">
...
>
	...
	{#if $files.items.length === 0}
		<p class="p-3 text-sm text-gray-500 dark:text-gray-300">
			No files
		</p>
	{:else}
			...
			{#each $files.items as file, i}
				<li class="group">
					<div class="flex">
						...
						<button class="hidden group-hover:block" onclick={() => {
...
<Modal show={fileToDelete !== undefined} msg={`Are you sure you want to delete ${fileToDelete}?`}
