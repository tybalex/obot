...
export function ModelProviderDeconfigure({
	...
	const deconfigure = useAsync(
		ModelProviderApiService.deconfigureModelProviderById,
		{
			...
			onError: () => toast.error(`Failed to deconfigure ${modelProvider.name}`),
		}
	);
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent hideCloseButton>
				...
				<p>
					Deconfiguring this model provider will remove all models associated
					with it and reset it to its unconfigured state. You will need to set
					up the model provider once again to use it.
				</p>

				<p>
					Are you sure you want to deconfigure <b>{modelProvider.name}</b>?
				</p>
