...
export default function AgentKnowledgePanel({
	...
	const startUpload = (files: FileList) => {
		...
		uploadKnowledge.execute(Array.from(files).map((file) => [file] as const));
		...
		<div className="flex flex-col items-center justify-center gap-4">
			{!hasDefaultTextEmbedding && (
				<WarningAlert
					...
					description={
						<p>
							In order to process the knowledge base for your agent, you&apos;ll
							need to set up a default text embedding model. Click{" "}
							<Link to={$path("/model-providers")}>here</Link> to update your
							model provider and/or default models.
						</p>
					}
				/>
			)}
			...
			<div className="flex w-full flex-col gap-2">
				...
				<AddKnowledgeButton
					...
					hasExistingNotion={knowledgeSources.some(
						(source) =>
							getKnowledgeSourceType(source) === KnowledgeSourceType.Notion
					)}
				/>
			</div>
			...
			{selectedKnowledgeSourceId && selectedKnowledgeSource && (
				<KnowledgeSourceDetail
					...
					onSyncNow={() => syncKnowledgeSource(selectedKnowledgeSourceId)}
					onDelete={() => deleteKnowledgeSource(selectedKnowledgeSourceId)}
					onUpdate={(source) =>
