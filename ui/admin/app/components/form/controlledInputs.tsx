					...
					<Input
						...
						onChange={(e) => {
							if (onChangeConversion) {
								e.target.value = onChangeConversion(e.target.value);
							}
			...
			render={({ field, fieldState }) => (
				<BasicInputItem label={label} description={description}>
					<Textarea
						{...field}
						{...inputProps}
						className={cn(getFieldStateClasses(fieldState), className)}
					/>
				</BasicInputItem>
			)}
		/>
...
function getFieldStateClasses(fieldState: ControllerFieldState) {
	return cn({
		"border-destructive focus-visible:ring-destructive": fieldState.invalid,
	});
}
