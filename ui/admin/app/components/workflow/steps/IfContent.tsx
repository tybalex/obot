...
export function IfContent({
	...
	const deleteNestedStep = (branch: "steps" | "else", index: number) => {
		const newBranch = (ifCondition[branch] || []).filter((_, i) => i !== index);
		onUpdate({ ...ifCondition, [branch]: newBranch });
	};
		...
		<div className="space-y-4 p-3">
			<div className="space-y-2">
				...
				<SortableList
					...
					getKey={(step) => step.id}
					onChange={(newSteps) => onUpdate({ ...ifCondition, steps: newSteps })}
					isHandle={false}
				/>
			...
			<div className="space-y-2">
				...
				<SortableList
					...
					getKey={(step) => step.id}
					onChange={(newSteps) => onUpdate({ ...ifCondition, else: newSteps })}
					isHandle={false}
				/>
